...
This markdown file has everything you need for the forage into grain "FIG" experiment. Data import, data analysis, figure generation, table generation and commentary along the way. 
...
pacman::p_load(
  ...
  agricolae, #standard anova
  nlme,      #when homogeneity of variance is violated
  lme4,      #mixed effect model, preferred
  car,       #Anova() comparison between mixed effect models
...
# sourcing my custom ggplot() theme
...
# Remember to switch chunk output from inline to in consolve for  figure generation
...
Box and whisker plots were used to identify outliers within environments.
...
# names(colors_treatments) <- levels(dat3$treatment)[1:9]
...
Split plot: amount of harvests per year
...
All responses are predicted using a global model [see Anderson and Burnham (2002)].

The global model is a linear mixed effect model with the following effect structure...
...
Each harvest systems exists within a unique  environment (fields and years)

We expect that each data point within a timepoint (i.e. yield of the first cut of a 3-cut harvest system in R100 in 2019) to already be an average of the plot since samples were collected by finding a representative area within the plot and sampling the population. 

Along with our raw data, we can generate seasoned data. It extends the raw data by adding columns like GDD accumulation or cumulative yield. 
...
ggplot(aes(x=yield.total, 
  ...
  stat_bin(bins = 30) +
  treatment_colors +
  scale_fill_manual(values = colors_treatments) +
  facet_wrap(~env,
             ncol = 1) +
...
rt.i.gg <- dat3 %>%
  ...
  mutate(follow.cut = fct_recode(follow.cut,
                                 `1` ="none",
                                 `2` = "september",
                                 `3` = "october")) %>%
  group_by(follow.cut) %>%
...
dat2.long <-  dat2.long %>%
  mutate(variable = fct_recode(variable,
             "October cut" = "yield.3cut",
             "September cut" = "yield.2cut",
             "Spring cut" = "yield.1cut")) %>%
  mutate(follow.cut = fct_recode(follow.cut,
             "september\n    +        \noctober  " = "october")) 
